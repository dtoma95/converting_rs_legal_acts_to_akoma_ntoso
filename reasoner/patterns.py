...
class FoundToken():
    def __init__(self, type, name, value, number, number2=None):
        self.type = type
        self.name = name
        self.value = value
        self.number = number
...
def is_glava(text):
    ...
    if m:
        return FoundToken(TokenType.GLAVA, "глава", None, roman_to_int(m.group(2)))

    m = re.match("(" + ROMAN_NUMERAL + ")(\.) (.*)" , text)
    if m:
        return FoundToken(TokenType.GLAVA, "глава", m.group(3), roman_to_int(m.group(1)))
...
def is_odeljak(text):
    m = re.match("([0-9]+)(\.)(.*)", text)
    if m:
        return FoundToken(TokenType.ODELJAK, "одељак", m.group(3), int(m.group(1)))
    return False
...
def is_pododeljak(text):
    ...
    if m:
        return FoundToken(TokenType.PODODELJAK, "пододељак", m.group(3), None)
    return False
...
def is_clan(text):
    ...
    if m:
        return FoundToken(TokenType.CLAN, "члан",None , int(m.group(2)))
    m = re.match("(Чл\.) ([0-9]+\-[0-9]+)(\.)", text)
    if m:
        br1 = int(m.group(2).split("-")[0])
        br2 = int(m.group(2).split("-")[1])
        return FoundToken(TokenType.CLAN, "члан", None, br1, br2)
    return False
...
def is_tacka(text):
    m = re.match("([0-9]+)(\))(.*)", text)
    if m:
        return FoundToken(TokenType.TACKA, "тачка", m.group(3), int(m.group(1)))
    return False

def is_podtacka(text):
    m = re.match("(\()([0-9]+)(\))(.*)", text)
    if m:
        return FoundToken(TokenType.PODTACKA, "подтачка", m.group(4), int(m.group(2)))
    return False
